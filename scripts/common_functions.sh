...
get_instance_health_elb() {
    ...
    local instance_registered=$($AWS_CLI elb describe-instance-health \
        --load-balancer-name $elb_name \
        --instances $instance_id \
        --query 'InstanceStates[].Description' \
        --output text 2>/dev/null)
    
    if [ $? == 0 ]; then
        if [ "$instance_registered" = "Instance is not currently registered with the LoadBalancer." ]; then
            return 1
        fi
    fi
...
validate_elb() {
    ...
    get_instance_health_elb $instance_id $elb_name
    if [ $? != 0 ]; then
        msg "no encontrado"
        return 1
    fi
